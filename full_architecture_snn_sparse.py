...
class SynapticWeightMemory:
    def __init__(self, input_dim, hidden_dim, w_min=0.0, w_max=1.0, prune_threshold=0.0, seed=0):
    ...
    def write_all(self, w):
        ...
        if self.prune_threshold > 0:
            w[np.abs(w) < self.prune_threshold] = 0.0
...
class SparseIndexController:
    """
    Indexed sparse fetch table:
    pre_neuron_id -> list of (post_id, weight) with |w| > threshold.
    """

    def __init__(self, index_threshold=1e-6):
        self.index_threshold = float(index_threshold)
        self.cols = None
        self.vals = None

    def rebuild(self, w, synapse_gate_mask=None):
        if synapse_gate_mask is not None:
            w = w * synapse_gate_mask
        self.cols = []
        self.vals = []
        for r in range(w.shape[0]):
            row = w[r]
            nz = np.where(np.abs(row) > self.index_threshold)[0].astype(np.int32)
            self.cols.append(nz)
            self.vals.append(row[nz].astype(np.float32))

    def accumulate(self, active_pre_ids, hidden_dim, neuron_gate_mask=None):
        current = np.zeros((hidden_dim,), dtype=np.float32)
        for ridx in active_pre_ids:
            cols = self.cols[int(ridx)]
            vals = self.vals[int(ridx)]
            if cols.size == 0:
                continue
            if neuron_gate_mask is not None:
                keep = neuron_gate_mask[cols] > 0
                if np.any(keep):
                    np.add.at(current, cols[keep], vals[keep])
            else:
                np.add.at(current, cols, vals)
        return current


class SynapseArray:
    def __init__(self, weight_mem: SynapticWeightMemory, use_sparse_index=False, index_threshold=1e-6, index_rebuild_interval=1):
        self.weight_mem = weight_mem
        self.synapse_gate_mask = None
        self.neuron_gate_mask = None

        self.use_sparse_index = bool(use_sparse_index)
        self.index_rebuild_interval = max(1, int(index_rebuild_interval))
        self._updates_since_rebuild = 0
        self.index_ctrl = SparseIndexController(index_threshold=index_threshold)
    ...
    def set_neuron_gate_mask(self, mask):
        self.neuron_gate_mask = mask.astype(np.uint8) if mask is not None else None

    def rebuild_index(self):
        w = self.weight_mem.read_all()
        self.index_ctrl.rebuild(w, synapse_gate_mask=self.synapse_gate_mask)
        self._updates_since_rebuild = 0

    def notify_weight_update(self):
        if not self.use_sparse_index:
            return
        self._updates_since_rebuild += 1
        if self._updates_since_rebuild >= self.index_rebuild_interval:
            self.rebuild_index()

    def accumulate(self, pre_spikes):
        active_pre_ids = np.where(pre_spikes > 0)[0]
        hidden_dim = self.weight_mem.mem.cols
        if len(active_pre_ids) == 0:
            return np.zeros((hidden_dim,), dtype=np.float32)

        if self.use_sparse_index:
            return self.index_ctrl.accumulate(active_pre_ids, hidden_dim, neuron_gate_mask=self.neuron_gate_mask)

        # Dense fallback: fetch only active pre rows.
        current = np.zeros((hidden_dim,), dtype=np.float32)
        for ridx in active_pre_ids:
            row = self.weight_mem.mem.read_row(int(ridx))
            if self.synapse_gate_mask is not None:
                row = row * self.synapse_gate_mask[int(ridx)]
            current += row.astype(np.float32)

        if self.neuron_gate_mask is not None:
            current *= self.neuron_gate_mask.astype(np.float32)
        return current
...
class NeuromorphicArchitecture:
    def __init__(
        ...
        index_threshold=1e-6,
        index_rebuild_interval=1,
        prune_threshold=0.0,
        seed=0,
    ):
        self.router = SpikeRouter()
        self.weight_mem = SynapticWeightMemory(
            input_dim,
            hidden_dim,
            w_min=w_min,
            w_max=w_max,
        ...
        self.synapse_array = SynapseArray(
            ...
            index_rebuild_interval=index_rebuild_interval,
        )
    ...
    def run_sample(self, x, train=False, use_gating=False):
        ...
        for t in range(t_steps):
            ...
            self.synapse_array.set_neuron_gate_mask(self.gating.neuron_gate_mask if use_gating else None)
            ...
            if train:
                ...
                self.synapse_array.notify_weight_update()
...
def train(args):
    ...
    if args.use_sparse_index:
        print(
            f"Sparse index enabled: threshold={args.index_threshold}, "
            f"rebuild_interval={args.index_rebuild_interval}, prune_threshold={args.prune_threshold}"
        )
    ...
    model = NeuromorphicArchitecture(
        ...
        use_sparse_index=args.use_sparse_index,
        index_threshold=args.index_threshold,
        index_rebuild_interval=args.index_rebuild_interval,
    ...
    np.savez_compressed(
        ...
        use_sparse_index=np.int64(1 if args.use_sparse_index else 0),
        index_threshold=np.float32(args.index_threshold),
        index_rebuild_interval=np.int64(args.index_rebuild_interval),
        prune_threshold=np.float32(args.prune_threshold),
    )
    ...
    metrics = {
        ...
        "hidden_dim": args.hidden_dim,
        "use_sparse_index": bool(args.use_sparse_index),
        "index_threshold": args.index_threshold,
        "index_rebuild_interval": args.index_rebuild_interval,
        "prune_threshold": args.prune_threshold,
    }
...
def predict(args):
    ...
    if args.use_sparse_index:
        model.synapse_array.rebuild_index()
    if "gate_mask" in m:
...
def parse_args():
    p = argparse.ArgumentParser(description="Full neuromorphic architecture: router + synapse SRAM + LIF + STDP + correlation gating + sparse indexed fetch")
    ...
    tr.add_argument("--use-sparse-index", action="store_true")
    tr.add_argument("--index-threshold", type=float, default=1e-6)
    tr.add_argument("--index-rebuild-interval", type=int, default=1)
    tr.add_argument("--prune-threshold", type=float, default=0.0)
    ...
    pr.add_argument("--use-sparse-index", action="store_true")
    pr.add_argument("--index-threshold", type=float, default=1e-6)
    pr.add_argument("--index-rebuild-interval", type=int, default=1)
    pr.add_argument("--prune-threshold", type=float, default=0.0)
